...
class SendStream extends EventEmitter {
  ...
  isPreconditionFailure(etag, lastModified) {
    ...
    if (match) {
      if (!isAnyEtagValue(match) && !hasMatchingEtag(match, etag, false)) {
        return true;
      }
    } else {
      ...
      if (!Number.isNaN(unmodifiedSince)) {
        ...
        if (timestamp > unmodifiedSince) {
          return true;
        }
      }
    }

    if (!isRevalidationMethod(this.req.method)) {
      const noneMatch = getOwnOption(this._headers, 'if-none-match');
      return Boolean(noneMatch && (isAnyEtagValue(noneMatch) || hasMatchingEtag(noneMatch, etag)));
    }
  ...
  isFresh(etag, lastModified) {
    return isRevalidationMethod(this.req.method) && isFresh(this._headers, etag, lastModified);
  }
  ...
  isRangeFresh(etag, lastModified) {
    ...
    if (ifRange.includes('"')) {
      return typeof etag === 'string' && !etag.startsWith('W/') && ifRange.trim() === etag;
    }
  ...
  send(filePath, stat, streamPath = filePath) {
    ...
    if (this.trySendConditionalResponse(filePath, stat)) {
      return;
    }
  ...
  sendOpenedFile(filePath, stat, fd) {
    ...
    if (this.isConditionalGET()) {
      if (this.isPreconditionFailure(etag, lastModified ?? stat.mtime.toUTCString())) {
        closeFile(fd);
        this.error(412);
        return;
      }
    ...
    if (req.method === 'GET' && this._acceptRanges && BYTES_RANGE_REGEXP.test(rangeHeader)) {
   ...
   * @returns {boolean}
   */
  trySendConditionalResponse(filePath, stat) {
    if (!this.isConditionalGET() || this.listenerCount('headers') !== 0) {
      return false;
    }

    const res = this.res;
    const lastModified = this._lastModified
      ? res.getHeader('Last-Modified') || stat.mtime.toUTCString()
      : res.getHeader('Last-Modified');
    const etag = this._etag
      ? res.getHeader('ETag') || createStatEntityTag(stat)
      : res.getHeader('ETag');
    const preconditionFailure = this.isPreconditionFailure(etag, lastModified ?? stat.mtime.toUTCString());
    const fresh = !preconditionFailure && this.isCachable() && this.isFresh(etag, lastModified);

    if (!preconditionFailure && !fresh) {
      return false;
    }
...
function isRevalidationMethod(method) {
