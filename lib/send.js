...
class SendStream extends EventEmitter {
  ...
  constructor(req, filePath, options) {
    ...
    this.options = normalized.streamOptions;
    this.path = filePath;
    this.req = req;
    this._headers = req.headers;
    this._acceptRanges = normalized.acceptRanges;
    this._cacheControl = normalized.cacheControl;
    this._cacheControlHeader = normalized.cacheControlHeader;
    this._conditionalGET = hasConditionalHeaders(this._headers);
    this._dotfiles = normalized.dotfiles;
    this._etag = normalized.etag;
    this._extensions = normalized.extensions;
    this._hasTrailingSlash = filePath[filePath.length - 1] === '/';
    this._index = normalized.index;
    this._lastModified = normalized.lastModified;
  ...
  send(filePath, stat) {
    ...
    this.stream(filePath, this.createStreamOptions(offset, len));
  }
