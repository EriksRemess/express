...
export default function readBody(req, res, next, parse, debug, options) {
  ...
  if (req.readableEncoding) {
    next(createError(500, "stream encoding should not be set", {
...
function decodeBuffer(buffer, charset) {
  // The generic UTF-16 label allows either byte order, identified by its BOM.
  if (charset === "utf-16" && buffer[0] === 0xfe && buffer[1] === 0xff) {
    charset = "utf-16be";
  }
