...
export default function readBody(req, res, next, parse, debug, options) {
  ...
  readBuffer(stream, req, length, options.limit, debug)
    .then(buffer => {
      if (options.verify) {
        try {
          ...
          throw createError(error.status ?? 403, error, {
            body: charset === null ? buffer : decodeBuffer(buffer, charset),
            type: error.type || "entity.verify.failed",
          });
        }
      }
      ...
      try {
        ...
        throw createError(400, error, {
          body,
          type: error.type || "entity.parse.failed",
        });
      }
    })
    .then(() => {
    ...
    .catch(error => {
      drainRequest(req);
      next(error.status ? error : createError(400, error));
    });
}
