...
export function compile(val) {
  ...
  for (let i = 0; i < trust.length; i++) {
    ...
    trusted.addSubnet(subnet.address, subnet.prefix, subnet.family);
  }
  ...
  return (addr) => {
    ...
    if (kind === 4) {
      return trusted.check(addr, 'ipv4');
    }
    ...
    return trusted.check(addr, 'ipv6');
  };
}
...
function forwarded(req) {
  ...
  const header = getOwnOption(req.headers, 'x-forwarded-for') || '';
