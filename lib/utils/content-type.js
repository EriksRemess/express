...
import { splitQuotedString } from '#lib/utils/http-parsing';
import mime from "mime-types";

// Package-derived from content-type/media-type parsing behavior. Keep security
// fixes compared with upstream before intentionally diverging from this copy.

const QUALITY_REGEXP = /^(?:0(?:\.\d*)?|1(?:\.0*)?|\.\d+)$/;
...
export function setCharset(type, charset) {
  ...
  const segments = splitParameters(String(type));
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (!value || !value.includes("=") || /^charset\s*=/i.test(value)) {
      continue;
    }
...
function splitParameters(value) {
  return splitQuotedString(value, ';').map(parameter => parameter.trim());
}

function parseQuality(value) {
  const quality = value.trim();
  if (!QUALITY_REGEXP.test(quality)) {
    return null;
  }
