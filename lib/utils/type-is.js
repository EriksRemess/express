...
export default function typeisRequest(req, ...types_) {
  ...
  return typeis(getOwnOption(req.headers, 'content-type'), types);
}
...
export function hasBody(req) {
  return getOwnOption(req.headers, 'transfer-encoding') !== undefined
    || isContentLength(getOwnOption(req.headers, 'content-length'));
}
...
function isContentLength(value) {
  return value !== undefined && /^\d+$/.test(String(value).trim());
}
