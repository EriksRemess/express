...
export function hasMatchingEtag(value, etag, weak = true) {
  if (!etag) {
    return false;
  }
  ...
  if (value.indexOf(',') === -1) {
    return etagMatches(trimHttpWhitespace(value), etag, weak);
  }
  ...
  for (let i = 0; i < value.length; i += 1) {
    ...
    if (code === 0x22) {
      quoted = !quoted;
    } else if (code === 0x2c && !quoted) {
      if (etagMatches(trimHttpWhitespace(value.slice(start, i)), etag, weak)) {
        return true;
      }
      start = i + 1;
    }
  ...
  return !quoted && etagMatches(trimHttpWhitespace(value.slice(start)), etag, weak);
}

function etagMatches(token, etag, weak) {
  if (!weak) {
    return typeof etag === 'string' && !etag.startsWith('W/') && token === etag;
  }
