...
// Package-derived from range-parser. Keep security fixes compared with upstream
// before intentionally diverging from this local copy.
const RANGE_SPEC_REGEXP = /^ *(\d*)-(\d*) *$/;
...
export default function parseRange(size, str, options) {
  ...
  for (let i = 0; i < parts.length; i++) {
    const range = RANGE_SPEC_REGEXP.exec(parts[i]);
