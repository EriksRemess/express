...
// Package-derived from cookie, cookie-signature, and cookie-parser behavior.
// Keep security fixes compared with upstream before intentionally diverging.
...
export function parseCookie(str, options) {
  ...
  const dec = getOwnOption(options, 'decode') || decode;
  let index = 0;
  ...
  do {
    ...
    if (!isUnsafeCookieName(key) && obj[key] === undefined) {
...
export function stringifyCookie(cookie, options) {
  const enc = getOwnOption(options, 'encode') || encodeURIComponent;
  const cookieStrings = [];
...
export function stringifySetCookie(nameOrCookie, valueOrOptions, maybeOptions) {
  ...
  const enc = getOwnOption(options, 'encode') || encodeURIComponent;
  const name = getOwnOption(cookie, 'name');
  const cookieValue = getOwnOption(cookie, 'value');

  if (!COOKIE_NAME_REGEXP.test(name)) {
    throw new TypeError(`argument name is invalid: ${name}`);
  }

  const value = cookieValue === undefined
    ? ''
    : enc(String(cookieValue));
  if (!COOKIE_VALUE_REGEXP.test(value)) {
    throw new TypeError(`argument val is invalid: ${cookieValue}`);
  }

  let str = `${name}=${value}`;

  const maxAge = getOwnOption(cookie, 'maxAge');
  if (maxAge !== undefined) {
  ...
  const domain = getOwnOption(cookie, 'domain');
  if (domain) {
    if (!DOMAIN_VALUE_REGEXP.test(domain)) {
  ...
  const path = getOwnOption(cookie, 'path');
  if (path) {
    if (!PATH_VALUE_REGEXP.test(path)) {
  ...
  const expires = getOwnOption(cookie, 'expires');
  if (expires) {
  ...
  if (getOwnOption(cookie, 'httpOnly')) {
    str += '; HttpOnly';
  }

  if (getOwnOption(cookie, 'secure')) {
    str += '; Secure';
  }

  if (getOwnOption(cookie, 'partitioned')) {
    str += '; Partitioned';
  }

  const priorityOption = getOwnOption(cookie, 'priority');
  if (priorityOption) {
    const priority = typeof priorityOption === 'string'
      ? priorityOption.toLowerCase()
      : undefined;
    ...
    switch (priority) {
      ...
      case 'high':
        ...
        break;
      default:
        throw new TypeError(`option priority is invalid: ${priorityOption}`);
    }
  }

  const sameSiteOption = getOwnOption(cookie, 'sameSite');
  if (sameSiteOption) {
    const sameSite = typeof sameSiteOption === 'string'
      ? sameSiteOption.toLowerCase()
      : sameSiteOption;
    ...
    switch (sameSite) {
      ...
      case 'none':
        ...
        break;
      default:
        throw new TypeError(`option sameSite is invalid: ${sameSiteOption}`);
    }
  }
...
export default function cookieParser(secret, options) {
  ...
  return function cookieParserMiddleware(req, res, next) {
    if (Object.hasOwn(req, 'cookies') && req.cookies) {
      if (!Object.hasOwn(req, 'secret')) {
    ...
    const cookieHeader = getOwnOption(req.headers, 'cookie');
...
function isUnsafeCookieName(name) {
