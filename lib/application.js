...
import { compileETag } from "#lib/utils/etag";
...
import { compileQueryParser } from "#lib/utils/query-parser";
import { compileTrust } from "#lib/utils/trust";
...
const trustProxyDefaultSymbol = Symbol('trust proxy default');
...
app.set = function set(setting, ...values) {
  if (values.length === 0) {
    // app.get(setting)
...
httpMethods.forEach(method => {
  app[method] = function methodHandler(path, ...handlers) {
...
app.render = function render(name, options, callback) {
  ...
  // merge options
  const renderOptions = Object.assign(Object.create(null), this.locals, getOwnOption(opts, '_locals'), opts);
...
app.listen = function listen(...args) {
  ...
  if (typeof args[args.length - 1] === 'function') {
    ...
    const done = args[args.length - 1] = once(function onListen(...callbackArgs) {
